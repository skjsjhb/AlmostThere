...
void luaInit()
{
    ...
    luaL_dofile(luaEngine, getAppResource("lualibs/preload.lua").c_str());
}
...
int luaGetInt(const std::string &varName)
{
    ...
    {
        warn("Incompatible Lua types detected. Reading '" + varName + "' as INT. It is " + lua_typename(luaEngine, lua_type(luaEngine, -1)));
    }
...
double luaGetNumber(const std::string &varName)
{
    ...
    {
        warn("Incompatible Lua types detected. Reading '" + varName + "' as NUMBER. It is " + lua_typename(luaEngine, lua_type(luaEngine, -1)));
    }
    lua_remove(luaEngine, -1);
    return r;
}
...
std::string luaGetStr(const std::string &varName)
{
    ...
    {
        warn("Incompatible Lua types detected. Reading '" + varName + "' as STR. It is " + lua_typename(luaEngine, lua_type(luaEngine, -1)));
    }
    lua_remove(luaEngine, -1);
    return r;
}
...
bool luaGetBool(const std::string &varName)
{
    ...
    {
        r = lua_toboolean(luaEngine, -1) == 0 ? false : true;
    }
    else
    {
        warn("Incompatible Lua types detected. Reading '" + varName + "' as BOOL. It is " + lua_typename(luaEngine, lua_type(luaEngine, -1)));
    }
    lua_remove(luaEngine, -1);
    return r;
}
...
StringTable luaGetTable(const std::string &varName)
{
    ...
    {
        warn("Incompatible Lua types detected. Reading '" + varName + "' as TABLE. It is " + lua_typename(luaEngine, lua_type(luaEngine, -1)));
    }
    lua_remove(luaEngine, -1);
    return StringTable();
}
