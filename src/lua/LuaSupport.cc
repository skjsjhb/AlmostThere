...
static void handleErr(int err) {
  if (err != 0) {
    switch (err) {
    case LUA_ERRMEM:error("Lua interpreter reported insufficient memory. Are there memory leaks?");
      break;
    case LUA_ERRSYNTAX:error("Lua syntax error detected. Check your code.");
      break;
    case LUA_ERRRUN:error("Lua runtime error detected. See message below.");
      break;
    default:break;
    }
...
int luaPrecompile(const std::string &code) {
  ...
  if ((err = luaL_loadstring(luaEngine, code.c_str())) != LUA_OK) {
    ...
    switch (err) {
    case LUA_ERRSYNTAX:warn("Syntax error detected. Check the script or its loader.");
      break;
    case LUA_ERRMEM:warn("Insufficient memory detected. Is there any memory leak, or high-consuming functions called?");
      break;
    default:warn("Unknown error occurred. Check if Lua library is corrupted.");
      break;
    }
    id = LUA_NOREF;
  } else {
