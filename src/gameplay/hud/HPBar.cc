...
HPDrainSection::HPDrainSection(double valueBegin, double valuePct, double t, HPBarVariant va)
{
    ...
    xend = xbegin + valuePct * HP_BAR_WIDTH;
    variant = va;
}
...
bool HPDrainSection::shouldUnload(double absTime)
{
...
void HPBar::setHP(unsigned int nv, bool animation)
{
    ...
    if (nv < hpCurrent && animation)
    {
        ...
        hpDrainEffects.push_back(HPDrainSection(v1, v2, t, variant));
    }
...
{
    auto t = game.mapTimer.getTime();
    unsigned int len = hpCurrent * HP_BAR_WIDTH / double(hpMax);
    ...
    1      3      5
    */
    float x2tex = len / double(HP_BAR_HEIGHT);
    Point pts[6] =
        {
            Point({x1, y2, 0}, {0, 1}),
            Point({x1, y1, 0}, {0, 0}),
    ...
    float healAmplifier = 0;
    glm::vec4 colorAmplifier = {1, 1, 1, 1};

    // Heal effect
    if ((t - healEffectBeginTime) <= HP_HEAL_TIME)
    {
        healAmplifier = 1 - (t - healEffectBeginTime) / double(HP_HEAL_TIME);
